...
class Player extends SpriteAnimationGroupComponent<PlayerState>
  ...
  // Property to expose _isJumping
  bool get isJumping => _isJumping;

  // Add a horizontal speed variable
  final double _stepDistance = 50; // Adjust this value to define the distance of each step

  // Add a variable to track horizontal movement
  final double _velocityX = 0;
  ...
  void moveLeft() {
    position.x -= _stepDistance;
    current = PlayerState.running; // Ensure player is in running state
  }
  ...
  /// Allows the player to jump manually, regardless of the tap event.
  void manualJump() {
    // Check if the player is on the ground before allowing the jump.
    if (!inAir) {
      // Set the current state to jumping.
      current = PlayerState.jumping;

      // Create a jump effect towards a normalized upward vector.
      final jumpEffect = JumpEffect(Vector2(0.35, -1)..scaleTo(500));

      // Play the jump sound effect.
      game.audioController.playSfx(SfxType.jump);

      // Add the jump effect to the player.
      add(jumpEffect);
    }
  }
}
